...
class msg_adminAdminController extends msg_admin {
    ...
    function updateMsg($args) {
        if(!$args) return new BaseObject(-1, 'invalid_msg');

        // 변수 검사
        $args->title = trim($args->title);
        $args->content = trim($args->content);
        if(!$args->message_srl) return new BaseObject(-1, 'invalid_msg');
        if(!$args->title) return new BaseObject(-1, 'msg_title_is_null');
        if(!$args->content) return new BaseObject(-1, 'msg_content_is_null');
        if(!$args->readed || !in_array($args->readed, array('Y','N'))) $args->readed = "N";
        if($args->readed != "Y" || $args->readed_date) $args->readed_date = "";
        if(!$args->message_type || !in_array($args->message_type, array('R','S','T'))) return new BaseObject(-1, 'msg_invalid_request');

        // 쪽지가 존재하는지 검사함
        $oMsgadminAdminModel = getAdminModel('msg_admin');
        $output = $oMsgadminAdminModel->getMsg($args->message_srl);
        $message = $output->data;
        if(!$message->message_srl) return new BaseObject(-1, 'invalid_msg');
    ...
    function procMsg_adminAdminDelete() {
        ...
        if(!$message || ($message_srl != $message->message_srl)) return new BaseObject(-1, 'invalid_msg');
    ...
    function procMsg_adminAdminAllDelete() {
        ...
        if(!$cart) return new BaseObject(-1, 'msg_cart_is_null');

        $cart_list = explode('|@|', $cart);
        if(!count($cart_list)) return new BaseObject(-1, 'msg_cart_is_null');
